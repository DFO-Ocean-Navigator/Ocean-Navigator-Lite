...
import {GetTimestampsPromise, GetDepthsPromise} from '../Remote/OceanNavigator';
...
const PlotPanel = (props) => {  
  ...
  useEffect(() => {
    if (props.selectedVariable) {
      GetTimestampsPromise(props.selectedDataset, props.selectedVariable).then(result => {
        setTimestamps(result.data)
        props.updateStartTime(result.data[0].id)
        props.updateEndTime(result.data[0].id)
      });
      GetDepthsPromise(props.selectedDataset, props.selectedVariable).then(result => {
  ...
  useEffect(() => {
    if (props.selectedVariable && depths.length === 0 && (selectedTab ==="profile" || selectedTab ==="transect")) {
  ...
  const starTimeDropdown = <div key='startTime'>
    <label>Start Time:</label>
    <Dropdown key='startTimeDropdown' data={timestamps} onChange={updateStartTime}/>
  </div>;
  const endTimeDropdown = <div key='endTime'>
    <label>End Time:</label>
    <Dropdown key='endTimeDropdown' data={timestamps} onChange={updateEndTime}/>
  </div>;
  const depthsDropdown = hasDepth ? <div key='depth'>
    <label>Depth:</label>
  ...
  const inputs = <div key='optionsDiv' style={{display: "flex"}}>
    {selectors}
  </div>
    ...
    <div >
        <Card>
          ...
          <Card.Body>
            ...
            >
              <Tab key='profile' eventKey='profile' title="Profile" disabled={!hasDepth}>
                {inputs}
              </Tab>
              <Tab key='ts' eventKey='timeseries' title='Virtual Mooring'>
                {inputs}
              </Tab>
              <Tab key='transect' eventKey='transect' title="Transect" disabled={!hasDepth}>
                {inputs}
              </Tab>
              <Tab key='area' eventKey='area' title="Area">
                {inputs}
              </Tab> 
             </Tabs>
          </Card.Body>
